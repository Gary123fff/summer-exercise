...
void ReadOBJ::drawFace() {
	...
	for (const auto& face : m_faces) {
		...
		GLfloat light_amb[] = { 0.0, 0.0, 0.0, 0.0 };
		...
		if (m_rendermode == 0) {
			for (int i = 0; i < face.count; i++) {
				...
				if(m_textures_mode==0)
					glTexCoord2f(m_textures[face.ti[i]].x, m_textures[face.ti[i] ].y);
				else {
					if (m_vertices[face.vi[i]].s == -1 && m_vertices[face.vi[i]].t == -1) {
						m_vertices[face.vi[i]].s = m_textures[face.ti[i]].x;
						m_vertices[face.vi[i]].t = m_textures[face.ti[i]].y;
					}
					glTexCoord2f(m_vertices[face.vi[i]].s, m_vertices[face.vi[i]].t);
				}
				glVertex3f(m_vertices[face.vi[i]].x, m_vertices[face.vi[i]].y, m_vertices[face.vi[i]].z);
			...
			glNormal3f(face.m_fnorm[0], face.m_fnorm[1], face.m_fnorm[2]);
			for (int i = 0; i < face.count; i++) {
				if (m_textures_mode == 0)
					glTexCoord2f(m_textures[face.ti[i]].x, m_textures[face.ti[i]].y);
				else {
